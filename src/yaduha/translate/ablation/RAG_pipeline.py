...
from yaduha.translate.ablation.ablation_tools import rag_tools, pipeline_tools, rag_pipeline_messages
...
class RagPipelineTranslator(Translator):
    ...
    def translate(self, sentence: str) -> Translation:
        start = time.time()
        response = translate_sentence(sentence, model=self.model, functions=functions, example_messages=rag_pipeline_messages, tools=rag_tools + pipeline_tools)
        end = time.time()
