...
from yaduha.translate.ablation.ablation_tools import rag_tools, pipeline_tools, full_translation_messages
...
class FullTranslator(Translator):
    ...
    def translate(self, sentence: str) -> Translation:
        start = time.time()
        response = translate_sentence(sentence, model=self.model, functions=functions, example_messages=full_translation_messages, tools=rag_tools + pipeline_tools)
        end = time.time()
